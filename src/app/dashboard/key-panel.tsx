      ...
      {freshKey ? (
        <>
          ...
          <p className="mt-3 text-xs text-neutral-400">
            After you close this page the value is gone from our side — only
            the prefix{" "}
            <code className="rounded bg-black/40 px-1.5 py-0.5">
              {freshKey.slice(0, 12)}…
            </code>{" "}
            stays visible. Lost it? Hit regenerate — it&apos;ll revoke the
            current key.
          </p>
        </>
      ) : initialPrefix ? (
        <>
          ...
          <p className="mt-3 text-xs text-neutral-500">
            The raw key was only visible when you generated it. If you&apos;ve
            lost it, click regenerate — the current one stops working
            immediately.
          </p>
        </>
      ) : (
        <>
          <div className="mb-3 text-neutral-300">
            You don&apos;t have a key yet. Generate one to start using the MCP.
          </div>
          <button
            onClick={rotate}
            disabled={pending}
