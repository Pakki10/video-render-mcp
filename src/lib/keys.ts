...
const KEY_PREFIX = "vrm_live_";

export interface GeneratedKey {
  /** Raw key — shown to the user exactly once. */
  raw: string;
  /** SHA-256 hex of the raw key — what we store. */
  hash: string;
  /** First 12 chars of the raw key — safe to persist for UI display. */
...
/**
 * Look up a live (non-revoked) ApiKey by its raw bearer value.
...
/**
 * Rotate the user's key: revoke any existing live keys, insert a new one,
 * return the raw value to the caller (which must show it exactly once).
 */
